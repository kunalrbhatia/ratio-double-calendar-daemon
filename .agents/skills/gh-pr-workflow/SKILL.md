...
```powershell
git checkout -b <branch-name>; git add .; git commit -m "<conventional-commit-msg>"; git push -u origin <branch-name>; gh pr create --title "<pr-title>" --body-file pr-body.txt; node .agents/skills/verify-pr-status/scripts/verify-checks.cjs
```
