...
describe('InstrumentManager', () => {
  ...
  test('loadInstruments from disk cache if exists and today', async () => {
    ...
    expect(manager.getInstrument('NIFTY', '09JUL2026', 19000, 'CE')).not.toBeNull();
  });
  ...
  test('loadInstruments downloads from API and filters properly', async () => {
    ...
    const mockScripMaster = [
      ...
      {
        token: '111',
        symbol: 'NIFTY09JUL26XXXXX', // no CE or PE
        name: 'NIFTY',
        expiry: '09JUL2026',
        strike: '19000.000000',
        lotsize: 50,
    ...
    expect(httpClient.request).toHaveBeenCalled();
    expect(manager.getInstrument('NIFTY', '09JUL2026', 19000, 'CE')).toBeDefined();
  ...
  test('getInstrument returns null on missing instrument', () => {
    expect(manager.getInstrument('NIFTY', '09JUL2026', 19000, 'CE')).toBeNull();
  });
